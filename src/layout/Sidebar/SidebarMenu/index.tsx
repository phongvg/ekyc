...
const SidebarMenu = () => {
  ...
  const shouldHideMenu = (mainMenu: sidebarMenuType) => {
    return mainMenu.Items.map((data) => data.title).every((tittles) => pinedMenu.includes(tittles || ""));
  };
  const { t } = useTranslation();
  return (
    <nav className='sidebar-main'>
      {wrapper === "horizontal-wrapper" || layoutName == "losangeles" || "singapore" ? (
        <div className={`left-arrow ${leftArrow ? "disabled" : ""}`} id='left-arrow' onClick={scrollToLeft}>
          <ArrowLeft />
        </div>
      ) : (
        ""
      ...
      <div
        id='sidebar-menu'
        style={wrapper === "horizontal-wrapper" || layoutName == "losangeles" || "singapore" ? { marginLeft: margin + "px" } : { margin: "0px" }}>
      ...
      {wrapper === "horizontal-wrapper" || layoutName == "losangeles" || "singapore" ? (
        <div className={`right-arrow ${rightArrow ? "disabled" : ""}`} onClick={scrollToRight}>
          <ArrowRight />
        </div>
      ) : (
        ""
